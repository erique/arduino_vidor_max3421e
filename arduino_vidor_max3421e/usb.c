...
uint8_t usb_InTransfer(ep_t *pep, uint16_t nak_limit, 
  ...
  pinMode((5u), OUTPUT); digitalWrite((5u), HIGH);
  ...
  *nbytesptr = 0;
  ...
  while( 1 ) { 
    ...
    if( rcode ) {
      pinMode((5u), OUTPUT); digitalWrite((5u), LOW);
      return( rcode );
    }
...
uint8_t usb_OutTransfer(ep_t *pep, uint16_t nak_limit, 
  ...
  pinMode((6u), OUTPUT); digitalWrite((6u), HIGH);
  ...
  nak_count ++;
  if( nak_limit && ( nak_count == nak_limit )) {
    pinMode((6u), OUTPUT); digitalWrite((6u), LOW);
    return( rcode );
  }
  break;
      case hrTIMEOUT:
...
uint8_t usb_ctrl_req(usb_device_t *dev, uint8_t bmReqType, 
  ...
  uint16_t	nak_limit;
    ...
    pinMode((4u), OUTPUT); digitalWrite((4u), HIGH);

  rcode = usb_set_address(dev, &(dev->ep0), &nak_limit);
  if (rcode) {
        debugf("failed to set address %d", rcode);
    return rcode;
}
  ...
  if( rcode )	{	//return HRSLT if not zero
    pinMode((4u), OUTPUT); digitalWrite((4u), LOW);
        debugf("failed to dispatch %d", rcode);
    return( rcode );
  }
  ...
  if( dataptr != NULL )	{
    ...
    //return error
    if( rcode )	{     pinMode((4u), OUTPUT); digitalWrite((4u), LOW); debugf("failed to transfer %d", rcode);
return( rcode ); }
  }
...
uint8_t usb_configure(uint8_t parent, uint8_t port, bool lowspeed) {
  uint8_t rcode = 0;
  debugf("%s(parent=%x port=%d lowspeed=%d)\n", __FUNCTION__, parent, port, lowspeed);
  ...
  if(i < USB_NUMDEVICES) {
    ...
    debugf("Setting addr %x\n", i+1);
    rcode = usb_set_addr(d, i+1);
    if(rcode) {
      debug_puts("failed to assign address");
      return rcode;
    }
    ...
    for(c=0;class_list[c];c++) {
      debugf("trying to init class %d\n", c);
      rcode = class_list[c]->init(d);

      if (!rcode) {
	d->klass = class_list[c];
	...
	return 0;
      ...
      debug_puts(" -> not accepted :-(");
    }
  } else
    debugf("no more free entries\n");

  debugf("unsupported device\n");
  return 0;
}
...
void usb_poll() {
  ...
  if(timer_get_msec() > poll) {
    ...
    uint8_t i;
    for (i=0; i<USB_NUMDEVICES; i++)
      if(dev[i].bAddress && dev[i].klass && dev[i].klass->poll)
	rcode = dev[i].klass->poll(dev+i);
}
    
    switch( usb_task_state ) {
    case USB_DETACHED_SUBSTATE_INITIALIZE:
;;    debugf("USB_DETACHED_SUBSTATE_INITIALIZE");
      usb_reset_state();
      
      // just remove everything ...
    uint8_t i;
      for (i=0; i<USB_NUMDEVICES; i++) {
	if(dev[i].bAddress && dev[i].klass) {
	  rcode = dev[i].klass->release(dev+i);
	  dev[i].bAddress = 0;
	}
      }
...
//    debugf("USB_DETACHED_SUBSTATE_WAIT_FOR_DEVICE");
      break;
    case USB_DETACHED_SUBSTATE_ILLEGAL:
;;    debugf("USB_DETACHED_SUBSTATE_ILLEGAL");
      break;
      
    case USB_ATTACHED_SUBSTATE_SETTLE:              //settle time for just attached device            
//    debugf("USB_ATTACHED_SUBSTATE_SETTLE");
      if( delay < timer_get_msec() ) 
	usb_task_state = USB_ATTACHED_SUBSTATE_RESET_DEVICE;
      break;
      
    case USB_ATTACHED_SUBSTATE_RESET_DEVICE:
;;    debugf("USB_ATTACHED_SUBSTATE_RESET_DEVICE");
      max3421e_write_u08( MAX3421E_HCTL, MAX3421E_BUSRST );              // issue bus reset
      usb_task_state = USB_ATTACHED_SUBSTATE_WAIT_RESET_COMPLETE;
      break;
      
    case USB_ATTACHED_SUBSTATE_WAIT_RESET_COMPLETE:
;;    debugf("USB_ATTACHED_SUBSTATE_WAIT_RESET_COMPLETE");
...
;;    debugf("USB_ATTACHED_SUBSTATE_WAIT_SOF");
