...
static uint8_t usb_hub_init(usb_device_t *dev) {
  ...
  if( rcode ) {
    debug_puts("failed to get device descriptor 1");
    return rcode;
  }
  ...
  if(rcode = usb_get_dev_descr( dev, sizeof(usb_device_descriptor_t), &buf.dev_desc )) {
    debug_puts("failed to get device descriptor 2");
    return rcode;
  }
  ...
  if (rcode) {
    debug_puts("failed to get hub descriptor");
    return rcode;
  }
  ...
  if (rcode) {
    debug_puts("failed to read configuration descriptor");
    return rcode;
  }
  ...
  if (rcode) {
    debugf("failed to set configuration to %d\n", buf.conf_desc.bConfigurationValue);
    return rcode;
  }
...
static void usb_hub_show_port_status(uint8_t port, uint16_t status, uint16_t changed) {
  debugf("Status of port %d:\n", port);

  if(status & USB_HUB_PORT_STATUS_PORT_CONNECTION)    debug_puts(" connected");
  if(status & USB_HUB_PORT_STATUS_PORT_ENABLE)        debug_puts(" enabled");
  if(status & USB_HUB_PORT_STATUS_PORT_SUSPEND)       debug_puts(" suspended");
  if(status & USB_HUB_PORT_STATUS_PORT_OVER_CURRENT)  debug_puts(" over current");
  if(status & USB_HUB_PORT_STATUS_PORT_RESET)         debug_puts(" reset");
  if(status & USB_HUB_PORT_STATUS_PORT_POWER)         debug_puts(" powered");
  if(status & USB_HUB_PORT_STATUS_PORT_LOW_SPEED)     debug_puts(" low speed");
  if(status & USB_HUB_PORT_STATUS_PORT_HIGH_SPEED)    debug_puts(" high speed");
  if(status & USB_HUB_PORT_STATUS_PORT_TEST)          debug_puts(" test");
  if(status & USB_HUB_PORT_STATUS_PORT_INDICATOR)     debug_puts(" indicator");

  debugf("Changes on port %d:\n", port);
  if(changed & USB_HUB_PORT_STATUS_PORT_CONNECTION)   debug_puts(" connected");
  if(changed & USB_HUB_PORT_STATUS_PORT_ENABLE)       debug_puts(" enabled");
  if(changed & USB_HUB_PORT_STATUS_PORT_SUSPEND)      debug_puts(" suspended");
  if(changed & USB_HUB_PORT_STATUS_PORT_OVER_CURRENT) debug_puts(" over current");
  if(changed & USB_HUB_PORT_STATUS_PORT_RESET)        debug_puts(" reset");
}

static uint8_t usb_hub_port_status_change(usb_device_t *dev, uint8_t port, hub_event_t evt) {
  usb_hub_info_t *info = &(dev->hub_info);

  debugf("status change on port %d, 0x%x\n", port, evt.bmEvent);
  ...
  case USB_HUB_PORT_EVENT_LS_CONNECT:
    debugf(" dev %x: port %d connect!\n", dev->bAddress, port);

    if (bResetInitiated) {
      debugf("reset already in progress\n");
      return 0;
    }

    //    timer_delay_msec(100);

    debugf("resetting port %d\n", port);
